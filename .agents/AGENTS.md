...
- **Deploy Principal & Espejo LinkedIn:** `git commit` + `git push origin main` → GitHub Pages auto-build → sitio live. NO se requieren uploads manuales.
- **GitHub Pages:** (`agutierrez11.github.io/agutierrezj-profile`): conectado a `agutierrez11/agutierrezj-profile` (branch `main`). Build automatizado con GitHub Actions (.github/workflows/deploy.yml). Output: `dist/public`.
- **Cloudflare Pages** (`cvagutierrezj2026.pages.dev`): secundario.
